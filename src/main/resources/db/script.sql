...
-- roles de usuario (solo estos tres en base; oftalmólogo, recepción, etc. en la app)
...
-- usuarios (identidad común; un rol por usuario con enum user_role)
...
-- pacientes (perfil clínico; datos personales en users)
CREATE TABLE hubble_db.patients
(
    id              UUID PRIMARY KEY      DEFAULT uuid_generate_v4(),
    ...
    updated_at      TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at      TIMESTAMP(3),
    CONSTRAINT fk_patients_user FOREIGN KEY (user_id) REFERENCES hubble_db.users (id),
    CONSTRAINT uq_patients_user_id UNIQUE (user_id)
);

CREATE UNIQUE INDEX uq_patients_document ON hubble_db.patients (document_number)
    WHERE deleted_at IS NULL;

-- doctores (perfil profesional; datos personales en users)
CREATE TABLE hubble_db.doctors
(
    id         UUID PRIMARY KEY      DEFAULT uuid_generate_v4(),
    user_id    UUID         NOT NULL,
    specialty  speciality_type       DEFAULT 'GENERAL',
    active     BOOLEAN               DEFAULT TRUE,
    created_at TIMESTAMP(3)          DEFAULT CURRENT_TIMESTAMP(3),
    updated_at TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at TIMESTAMP(3),
    CONSTRAINT fk_doctors_user FOREIGN KEY (user_id) REFERENCES hubble_db.users (id),
    CONSTRAINT uq_doctors_user_id UNIQUE (user_id)
);

-- sesiones de usuario (refresh hash, JTI de access, dispositivo y vigencia)
...
(
    ...
    updated_at       TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at       TIMESTAMP(3),
    CONSTRAINT fk_appointment_patient FOREIGN KEY (patient_id) REFERENCES hubble_db.patients (id),
    CONSTRAINT fk_appointment_doctor FOREIGN KEY (doctor_id) REFERENCES hubble_db.doctors (id)
);
...
(
    ...
    updated_at      TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at      TIMESTAMP(3),
    CONSTRAINT fk_exam_patient FOREIGN KEY (patient_id) REFERENCES hubble_db.patients (id),
...
-- tarjetas guardadas (solo cuando el pago va por CARD; token desde cliente y pasarela)
...
(
    ...
    updated_at           TIMESTAMP(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at           TIMESTAMP(3),
    CONSTRAINT fk_payment_cards_patient FOREIGN KEY (patient_id) REFERENCES hubble_db.patients (id),
...
(
    ...
    updated_at       TIMESTAMP(3)   NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    deleted_at       TIMESTAMP(3),
    CONSTRAINT fk_payment_patient FOREIGN KEY (patient_id) REFERENCES hubble_db.patients (id),
...
-- notificaciones (mismo título y cuerpo para correo y app; cada canal marca su envío)
...
-- índices
CREATE INDEX idx_patient_document ON hubble_db.patients (document_number);
CREATE INDEX idx_appointment_date ON hubble_db.appointments (appointment_date);
CREATE INDEX idx_payment_cards_patient ON hubble_db.payment_cards (patient_id);
